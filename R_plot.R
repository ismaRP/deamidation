...
fn <- "palaeoproteomics/pompeii_pos/out/Deamidation.txt"
...
fn <- "palaeoproteomics/pompeii_pos/out/Number_of_Peptides_per_RawFile.txt"
...
plt <- ggplot(aes(y=mean, x=N_Q, fill=N_Q), data=df) + 
  ...
  labs(title="Deamidation per RawFile STD as error bars") +
  xlab("Asparagine Glutamine") + ylab("percent deamidation") + 
